...
__global__ void padding_preprocess(float* input_featuremap_data, float* input_featuremap_after_padding, 
...
__global__ void im2col_preprocess(float* input_featuremap_after_padding_data, float* b_matrix_after_im2col, 
                             int b_row, int b_col, int filt_h, int filt_w, int out_h, int out_w, int str_h, 
                             int str_w, int in_h_after_pad, int in_w_after_pad, int in_c, int warp_num_per_block){
    // index of thread in a warp, one warp has 1x32 threads
    const uint32_t lane_id = threadIdx.x % 32;
    // index of wrap in a thread block, one warp has 1x32 threads
    const uint32_t warp_id = threadIdx.x / 32;
    ...
    const uint32_t pixel_row_in_image = pixel_row_in_outimage*str_h + pixel_row_in_window;
    const uint32_t pixel_col_in_image = pixel_col_in_outimage*str_w + pixel_col_in_window;

    uint32_t o_addr = pixel_in_batch_of_image*in_c*in_h_after_pad*in_w_after_pad + 
                      pixel_in_channel_of_image*in_h_after_pad*in_w_after_pad + 
                      pixel_row_in_image*in_w_after_pad + pixel_col_in_image;
    
    if(row_b < b_row && col_b < b_col){
        *(b_matrix_after_im2col + row_b*b_col + col_b) = *(input_featuremap_after_padding_data+o_addr);
    }
}
...
Result test_preprocess(Options &options) {
    ...
    int input_featuremap_size_after_padding = in_h_after_pad * in_w_after_pad * in_c * in_n;
    
    // Declare the input featuremap after padding, and sync.
    Tensor4d<DTypeOutput> tensor_b_after_padding(input_featuremap_size_after_padding);
    tensor_b_after_padding.sync_device();
    
    // Call .device_data() to get device_addr.
    float *input_featuremap_after_padding_data = tensor_b_after_padding.device_data();  //device addr
    ...
    // |       |       |       |       |
    // ---------------------------------
    dim3 grid_naive_padding((in_w_after_pad + 31) / 32, (in_h_after_pad*in_c*in_n + warp_num_per_block-1) / warp_num_per_block);
    
    // warm up
    padding_preprocess<<<grid_naive_padding, warp_num_per_block*32>>>(
            input_featuremap_data, input_featuremap_after_padding_data, warp_num_per_block, 
            in_n, in_c, in_h, in_w, pad_h, pad_w, 0, 0, in_h_after_pad, in_w_after_pad);
    ...
    float* host_input_featuremap_after_padding;
    CUDA_CALL(cudaMallocHost(&host_input_featuremap_after_padding, input_featuremap_size_after_padding * sizeof(float)));
    CUDA_CALL(cudaMemcpy(host_input_featuremap_after_padding, input_featuremap_after_padding_data, 
              input_featuremap_size_after_padding * sizeof(float), cudaMemcpyDeviceToHost));
    
#ifdef PRINT
    // PRINT is defined in config.h, to see host data of tensor_b after padding.
    printf("\n###########Host After Padding Start###########\n");
    for(size_t i = 0; i < in_n; ++i){
        printf("=====================%d-th image\n", i);
        for(size_t j = 0; j < in_c; ++j){
            printf("--------------------%d-th channel\n", j);
            for(size_t k = 0; k < in_h_after_pad; ++k){
                for(size_t l = 0; l < in_w_after_pad; ++l){
                    printf("%.3f ", *(host_input_featuremap_after_padding+
                    i*in_h_after_pad*in_w_after_pad*in_c+j*in_h_after_pad*
                    in_w_after_pad+k*in_w_after_pad+l));
                }
                printf("\n");
            }
        }
    ...
    // Get time assumed by external padding or internal padding.
...
#ifdef DEBUG
    printf("\033[31mPRE Forward Padding time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
    ...
    // warm up
    im2col_preprocess<<<grid_naive_im2col, warp_num_per_block*32>>>(input_featuremap_after_padding_data, 
...
#ifdef DEBUG
    printf("\033[31mPRE Forward Im2col time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
...
#ifdef DEBUG
    printf("\033[31mPRE Forward MMA time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif

#ifdef DEBUG
    printf("\033[31mPRE Forward workload: %ld.\033[0m\n", workload);
    printf("\033[31mPRE Forward Total time: %lf ms.\033[0m\n", result.runtime_ms);
    printf("\033[31mPRE Forward Total gflops: %lf gflops.\033[0m\n", result.gflops);
    printf("\033[31mPRE Forward M = %d, K = %d, N = %d.\033[0m\n", m, k, n);
#endif
    ...
    input_featuremap_size_after_padding = in_h_after_pad * in_w_after_pad * in_c * in_n;

    // Declare LossNextLayer as Input feature map of Loss, loss of next layer.
    Tensor4dParam LossNextLayer_shape_after_pad(in_n, in_c, in_h_after_pad, in_w_after_pad);
    Tensor4d<DTypeOutput> LossNextLayer_after_padding(LossNextLayer_shape_after_pad);
    LossNextLayer_after_padding.init_host_data_random(); 
    LossNextLayer_after_padding.sync_device();

    // Call .device_data() to get device_addr.
    input_featuremap_after_padding_data = LossNextLayer_after_padding.device_data();  //device addr
    ...
    // |       |       |       |       |
    // ---------------------------------
    dim3 grid_naive_padding_loss((in_w_after_pad + 31) / 32, (in_h_after_pad*in_c*in_n + warp_num_per_block-1) / warp_num_per_block);
    
    // warm up
    padding_preprocess<<<grid_naive_padding_loss, warp_num_per_block*32>>>(
            input_featuremap_data, input_featuremap_after_padding_data, warp_num_per_block, 
            in_n, in_c, in_h, in_w, pad_h, pad_w, options.conv_stride.get_M()-1, 
            options.conv_stride.get_N()-1, in_h_after_pad, in_w_after_pad);
...
#ifdef PRINT
    // PRINT is defined in config.h, to see host data of LossNextLayer after padding.
    printf("\n###########Host After Padding Start###########\n");
    for(size_t i = 0; i < in_n; ++i){
        printf("=====================%d-th image\n", i);
        for(size_t j = 0; j < in_c; ++j){
    ...
    result.workspace += 0;
#ifdef DEBUG
    printf("\033[31mPRE Loss Padding time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
    ...
    // warm up
    im2col_preprocess<<<grid_naive_im2col_loss, warp_num_per_block*32>>>(input_featuremap_after_padding_data, 
            b_matrix_after_im2col, b_row, b_col, filt_h, filt_w, out_h, out_w, str_h, str_w,
            in_h_after_pad, in_w_after_pad, in_c, warp_num_per_block);
...
#ifdef DEBUG
    printf("\033[31mPRE Loss Im2col time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
...
#ifdef DEBUG
    printf("\033[31mPRE Loss MMA time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif

#ifdef DEBUG
    printf("\033[31mPRE Loss workload: %ld.\033[0m\n", workload);
    printf("\033[31mPRE Loss Total time: %lf ms.\033[0m\n", result.runtime_ms);
    printf("\033[31mPRE Loss Total gflops: %lf gflops.\033[0m\n", result.gflops);
    printf("\033[31mPRE Loss M = %d, K = %d, N = %d.\033[0m\n", m, k, n);
#endif
    //#######################LOSS END#############################
    ...
    // warm up
    padding_preprocess<<<gradient_grid_naive_padding, warp_num_per_block*32>>>(
...
#ifdef DEBUG
    printf("\033[31mPRE Gradient LossNextLayer Padding time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif

    // 2. Inputfeaturemap padding.
    // Record time, ms.
    all_time = 0;
    for (auto & event : events) {
      cudaEventCreate(&event);
    ...
    // gradient_in_h = in_h;
    // gradient_in_w = in_w;
    dim3 gradient_inputfeaturemap_grid_naive_padding((gradient_in_w+2*pad_w + 31) / 32, 
         ((gradient_in_h+2*pad_h)*gradient_in_c*
         gradient_in_n + warp_num_per_block-1) / warp_num_per_block);
    
    // warm up
    padding_preprocess<<<gradient_inputfeaturemap_grid_naive_padding, warp_num_per_block*32>>>(
            d_Inputfeaturemap_data, d_Inputfeaturemap_after_padding_data, warp_num_per_block, 
            gradient_in_n, gradient_in_c, gradient_in_h, gradient_in_w, pad_h, pad_w, 
            0, 0, gradient_in_h+2*pad_h, gradient_in_w+2*pad_w);
    ...
    // Copy the input featuremap map after padding back to host.
    float* host_Inputfeaturemap_after_padding_data_after_padding;
    CUDA_CALL(cudaMallocHost(&host_Inputfeaturemap_after_padding_data_after_padding, 
              gradient_in_n * gradient_in_c * (gradient_in_h+2*pad_h) * 
              (gradient_in_w+2*pad_w) * sizeof(float)));
    CUDA_CALL(cudaMemcpy(host_Inputfeaturemap_after_padding_data_after_padding, 
              d_Inputfeaturemap_after_padding_data, gradient_in_n * gradient_in_c * 
              (gradient_in_h+2*pad_h) * (gradient_in_w+2*pad_w) * sizeof(float), cudaMemcpyDeviceToHost));
...
#ifdef PRINT
    // PRINT is defined in config.h, to see host data of host_Inputfeaturemap_data_after_padding.
    printf("\n###########Inputfeaturemap_after_padding - Host After Padding Start###########\n");
    for(size_t i = 0; i < gradient_in_n; ++i){
        printf("=====================%d-th image\n", i);
        for(size_t j = 0; j < gradient_in_c; ++j){
            printf("--------------------%d-th channel\n", j);
            for(size_t k = 0; k < gradient_in_h+2*pad_h; ++k){
                for(size_t l = 0; l < gradient_in_w+2*pad_w; ++l){
                    printf("%.3f ", *(host_Inputfeaturemap_after_padding_data_after_padding+
                    i*(gradient_in_h+2*pad_h)*(gradient_in_w+2*pad_w)*
                    gradient_in_c+j*(gradient_in_h+2*pad_h)*
                    (gradient_in_w+2*pad_w)+k*(gradient_in_w+2*pad_w)+l));
                }
                printf("\n");
            }
        }
    }
    printf("###########Inputfeaturemap_after_padding - Host After Padding End#############\n\n");
#endif

#ifdef DEBUG
    printf("\033[31mPRE Gradient Inputfeaturemap Padding time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
    ...
    // warm up
    im2col_preprocess<<<grid_naive_im2col_gradient, warp_num_per_block*32>>>(d_Inputfeaturemap_after_padding_data, 
            b_matrix_after_im2col, b_row, b_col, out_h+(out_h-1)*(str_h-1), out_w+(out_w-1)*(str_w-1), 
            gradient_out_h, gradient_out_w, 1, 1, gradient_in_h+2*pad_h, gradient_in_w+2*pad_w, gradient_in_c, 
            warp_num_per_block);
...
#ifdef DEBUG
    printf("\033[31mPRE Gradient Im2col time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif
...
#ifdef DEBUG
    printf("\033[31mPRE Gradient MMA time: %lf ms.\033[0m\n", float(all_time)/options.iterations);
#endif

#ifdef DEBUG
    printf("\033[31mPRE Gradient workload: %ld.\033[0m\n", workload);
    printf("\033[31mPRE Gradient Total time: %lf ms.\033[0m\n", result.runtime_ms);
    printf("\033[31mPRE Gradient Total gflops: %lf gflops.\033[0m\n", result.gflops);
    printf("\033[31mPRE Gradient M = %d, K = %d, N = %d.\033[0m\n", m, k, n);
#endif
    //#######################GRADIENT END#########################
    return result;
